...
export function useGetMachine(machineId: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await atLicense.machines.get({ id: machineId })
...
export function useListMachines(
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const response = await atLicense.machines.list(
        params
          ? {
...
export function useCreateMachine() {
  ...
  return useMutation<Machine, APIError, Schemas.Machines.CreateValues>({
    mutationFn: async (values) => {
      const response = await atLicense.machines.create(values)
...
export function useUpdateMachine(machineId: string) {
  ...
  return useMutation<Machine, APIError, Schemas.Machines.UpdateValues>({
    mutationFn: async (values) => {
      ...
      const getResponse = await atLicense.machines.get({ id: machineId })
      ...
      const updateResponse = await atLicense.machines.update({
        id: machineId,
        values: changes,
      })
...
export function useRemoveMachine(machineId: string) {
  ...
  return useMutation({
    mutationFn: () => atLicense.machines.remove({ id: machineId }),
...
export function useResetMachineHeartbeat(machineId: string) {
  ...
  return useMutation<Machine, APIError>({
    mutationFn: async () => {
      const response = await atLicense.machines.resetHeartbeat({ id: machineId })
...
export function useChangeMachineOwner() {
  ...
  >({
    mutationFn: async ({ machineId, ownerId }) => {
      const response = await atLicense.machines.changeOwner({
        id: machineId,
        ownerId,
      })
...
export function useChangeMachineGroup() {
  ...
  >({
    mutationFn: async ({ machineId, groupId }) => {
      const response = await atLicense.machines.changeGroup({
        id: machineId,
        groupId,
      })
