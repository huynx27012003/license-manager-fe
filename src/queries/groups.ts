...
export function useGetGroup(groupId: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await atLicense.groups.get({ id: groupId })
...
export function useListGroups(
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const response = await atLicense.groups.list(
...
export function useCreateGroup() {
  ...
  return useMutation<Group, APIError, Schemas.Groups.CreateValues>({
    mutationFn: async (values) => {
      const response = await atLicense.groups.create(values)
...
export function useUpdateGroup(groupId: string) {
  ...
  return useMutation<Group, APIError, Schemas.Groups.UpdateValues>({
    mutationFn: (values) =>
      atLicense.groups.get({ id: groupId }).then(async (response) => {
        if (response.errors) {
          throw new APIError(response.errors[0])
        }
        ...
        const updateResponse = await atLicense.groups.update({
          id: groupId,
          values: changes,
        })
...
export function useRemoveGroup(groupId: string) {
  ...
  return useMutation({
    mutationFn: () => atLicense.groups.remove({ id: groupId }),
