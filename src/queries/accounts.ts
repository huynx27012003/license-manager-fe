...
export function useGetAccount() {
  return useQuery({
    queryKey: ["account"],
    queryFn: async () => {
      const response = await atLicense.accounts.get()
...
export function useUpdateAccount() {
  ...
  return useMutation<Account, APIError, MutationValues>({
    mutationFn: async (values) => {
      const getResponse = await atLicense.accounts.get()
      ...
      const updateResponse = await atLicense.accounts.update({
        values: changes,
      })
...
export function useGetAccountPlan(planId?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await atLicense.accounts.plan(planId!)
...
export function useGetAccountBilling(billingId?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await atLicense.accounts.billing()
...
export function useManageSubscription() {
  return useMutation<string, APIError>({
    mutationFn: async () => {
      const response = await atLicense.accounts.manageSubscription()
...
export function useCancelSubscription() {
  ...
  return useMutation<void, APIError>({
    mutationFn: async () => {
      const response = await atLicense.accounts.cancelSubscription()
...
export function useGetAccountSettings() {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await atLicense.accounts.settings()
...
export function useUpdateAccountPermissions() {
  ...
  return useMutation<void, APIError, Schemas.Account.PermissionsValues>({
    mutationFn: async (values) => {
      ...
      const settingsResponse = await atLicense.accounts.settings()
      ...
      if (defaultUserPermissions != null) {
        ...
        if (existing && defaultUserPermissions.length === 0) {
          await atLicense.accounts.removeSetting({ id: existing.id })
        } else if ((defaultUserPermissions?.length ?? 0) > 0) {
          const response = await atLicense.accounts.updateSetting({
            id: existing?.id,
