...
export function useGetProduct(productId: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await atLicense.products.get({ id: productId })
...
export function useListProducts(
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const response = await atLicense.products.list(
...
export function useCreateProduct() {
  ...
  return useMutation<Product, APIError, Schemas.Products.CreateValues>({
    mutationFn: (values) =>
      atLicense.products.create({ values }).then((response) => {
        if (response.errors) {
          throw new APIError(response.errors[0])
        }
...
export function useUpdateProduct(productId: string) {
  ...
  return useMutation<Product, APIError, Schemas.Products.UpdateValues>({
    mutationFn: (values) =>
      atLicense.products.get({ id: productId }).then(async (response) => {
        if (response.errors) {
          throw new APIError(response.errors[0])
        }
        ...
        const updateResponse = await atLicense.products.update({
          id: productId,
          values: changes,
        })
...
export function useRemoveProduct(productId: string) {
  ...
  return useMutation({
    mutationFn: () => atLicense.products.remove({ id: productId }),
