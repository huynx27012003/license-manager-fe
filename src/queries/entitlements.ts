...
export function useGetEntitlement(entitlementId: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await atLicense.entitlements.get({ id: entitlementId })
...
export function useListEntitlements(
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const response = await atLicense.entitlements.list(
...
export function useCreateEntitlement() {
  ...
  return useMutation<Entitlement, APIError, Schemas.Entitlements.CreateValues>({
    mutationFn: async (values) => {
      const response = await atLicense.entitlements.create({ values })
...
export function useUpdateEntitlement(entitlementId: string) {
  ...
  return useMutation<Entitlement, APIError, Schemas.Entitlements.UpdateValues>({
    mutationFn: (values) =>
      atLicense.entitlements.get({ id: entitlementId }).then(async (response) => {
        if (response.errors) {
          throw new APIError(response.errors[0])
        }
        ...
        const updateResponse = await atLicense.entitlements.update({
          id: entitlementId,
          values: changes,
        })
...
export function useRemoveEntitlement(entitlementId: string) {
  ...
  return useMutation({
    mutationFn: () => atLicense.entitlements.remove({ id: entitlementId }),
