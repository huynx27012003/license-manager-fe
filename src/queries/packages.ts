...
export function useGetPackage(packageId: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await atLicense.packages.get({ id: packageId })
...
export function useListPackages(
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const response = await atLicense.packages.list(
        params
          ? {
...
export function useCreatePackage() {
  ...
  return useMutation<Package, APIError, Schemas.Packages.CreateValues>({
    mutationFn: (values) =>
      atLicense.packages.create(values).then((response) => {
        if (response.errors) {
          throw new APIError(response.errors[0])
        }
...
export function useUpdatePackage(packageId: string) {
  ...
  return useMutation<Package, APIError, Schemas.Packages.UpdateValues>({
    mutationFn: (values) =>
      atLicense.packages.get({ id: packageId }).then(async (response) => {
        if (response.errors) {
          throw new APIError(response.errors[0])
        }
        ...
        const updateResponse = await atLicense.packages.update({
          id: packageId,
          values: changes,
        })
...
export function useRemovePackage(packageId: string) {
  ...
  return useMutation({
    mutationFn: () => atLicense.packages.remove({ id: packageId }),
