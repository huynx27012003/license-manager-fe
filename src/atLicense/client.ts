...
export class Client {
  ...
  private hydrateFromStorage() {
    if (typeof window === "undefined" || this.rootToken) return
    ...
    if (token) this.setRootToken(token)
  ...
  async request<T = unknown>(
    ...
    const defaultHeaders = {
      ...
      "AtLicense-Version": config.version,
      ...(authToken && !config.isCloud
        ? { Authorization: `Bearer ${authToken}` }
        : {}),
      ...(options.root || !this.environment
        ? {}
        : { "AtLicense-Environment": this.environment }),
    }
    ...
    if (response.status === 401 && "errors" in data) {
      ...
      if (
        ...
        window.dispatchEvent(new CustomEvent("atLicense:session-expired"))
      }
    }
