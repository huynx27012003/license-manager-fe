...
export function useGetArtifact(artifactId: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await atLicense.artifacts.get({ id: artifactId })
...
export function useListArtifacts(
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const response = await atLicense.artifacts.list(
        params
          ? {
...
export function useCreateArtifact() {
  ...
  return useMutation<Artifact, APIError, Schemas.Artifacts.CreateValues>({
    mutationFn: async (values) => {
      const response = await atLicense.artifacts.create(values)
...
export function useUpdateArtifact(artifactId: string) {
  ...
  return useMutation<Artifact, APIError, Schemas.Artifacts.UpdateValues>({
    mutationFn: (values) =>
      atLicense.artifacts.get({ id: artifactId }).then(async (response) => {
        if (response.errors) {
          throw new APIError(response.errors[0])
        }
        ...
        const updateResponse = await atLicense.artifacts.update({
          id: artifactId,
          values: changes,
        })
...
export function useRemoveArtifact(artifactId: string) {
  ...
  return useMutation({
    mutationFn: () => atLicense.artifacts.remove({ id: artifactId }),
