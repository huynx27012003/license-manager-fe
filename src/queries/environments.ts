...
export function useListEnvironments(params?: {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const response = await atLicense.environments.list(
...
export function useCreateEnvironment() {
  ...
  return useMutation<Environment, APIError, Schemas.Environments.CreateValues>({
    mutationFn: async (values) => {
      const response = await atLicense.environments.create({ values })
...
export function useUpdateEnvironment(environmentId: string) {
  ...
  return useMutation<Environment, APIError, Schemas.Environments.UpdateValues>({
    mutationFn: (values) =>
      atLicense.environments.get({ id: environmentId }).then(async (response) => {
        if (response.errors) {
          throw new APIError(response.errors[0])
        }
        ...
        const updateResponse = await atLicense.environments.update({
          id: environmentId,
          values: changes,
        })
...
export function useRemoveEnvironment(environmentId: string) {
  ...
  return useMutation({
    mutationFn: () => atLicense.environments.remove({ id: environmentId }),
